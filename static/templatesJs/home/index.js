...
function getRecommendArts() {
  ...
  .done(function(resp) {
    ...
    if (resp.status == "success") {
      ...
      var obj = "<div id='bookAll' class='col-md-12 col-sm-12 alert alert-success'>";
      ...
      for (var i in bookAll) {
        obj = obj + "<tr><td>" + (i+1) + "</td><td><a href='/chapter/bookChapter/?idBook= " + bookAll[i].id + "&bookName=" + bookAll[i].name + "'>" + bookAll[i].id + "</a></td><td>" + bookAll[i].name + "</td><td>" + bookAll[i].idAuthor_id + "</td><td> " + bookAll[i].chapterCount  + "</td></tr>";
      }
